...
export default function TVShowScreen({ route }) {
    ...
    function fetchAllData() {
        setIsLoading(true)
        axios.get(`https://api.themoviedb.org/3/tv/${showId}/season/${seasonNumber}?api_key=${TMDB_API_KEY}`)
            .then(d => {
                //insert data into their own state
                setSeasonData(d.data)

                //get overall data for the show (not just the season)
                axios.get(`https://api.themoviedb.org/3/tv/${showId}?api_key=${TMDB_API_KEY}`)
                    .then(d => setShowData(d.data)).catch(e => setIsError(true))
            ...
            >
                ...
                <View style={{ marginBottom: 15 }}>
                    ...
                    <Text style={{ ...styles.text, textAlign: 'left' }}>
                        {seasonData.overview ? seasonData.overview : `We don't have a description for this season yet.`}
                    </Text>
                </View>
