...
export default function Trending() {
  ...
  function fetchData() {
    fetch(
      ...
      result.json().then(data => setTrendingMovies(data.results)),
    );
  }
    ...
    <Animated.View style={styles.container}>
      ...
      {trendingMovies.length > 0 ? (
        <Pressable
          ...
          onPress={() =>
            navigation.push('movie', {movieId: trendingMovies[activeSlide].id})
          }>
          <IndexDots data={trendingMovies} active={activeSlide} />
          <Animated.Text
            style={{
              ...styles.title,
