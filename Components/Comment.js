...
export default function Comment({comment}) {
  ...
  function filterComment() {
    var removeHtmlRegex = /(<([^>]+)>)/gi;
    var urlCheckRegex = new RegExp(
      '([a-zA-Z0-9]+://)?([a-zA-Z0-9_]+:[a-zA-Z0-9_]+@)?([a-zA-Z0-9.-]+\\.[A-Za-z]{2,4})(:[0-9]+)?([^ ])+',
    ).test(comment.content);
    if (
      lowQualityCommenters.includes(
        comment.author_details.username.toLowerCase(),
      ) ||
      urlCheckRegex
    ) {
      setCommentHidden(true);
      setCommentText(
        'This comment has been automatically hidden because our systems tagged it as being low quality and/or harmful',
      );
      return;
    }
    setCommentText(
        ...
        <View style={styles.authorContainer}>
          <Text style={styles.author}>
            {comment.author_details
              ? comment.author_details.username.toLowerCase().replace(' ', '')
              : comment.author}{' '}
            thinks:
          </Text>
        </View>
